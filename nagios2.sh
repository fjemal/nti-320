...
htpasswd -b /etc/nagios/passwd nagiosadmin nti320
...
# Define a service to ping the cacti-server  machine
...
# Define a service to check HTTP on the cacti-server machine.
...
# on the cacti-server machine.  Warning if < 20% free, critical if
...
# users on the cacti-server machine.  Warning if > 20 users, critical
# if > 50 users.
...
# on the cacti-server machine.  Warning if > 250 processes, critical if
# > 400 processes.
...
# Define a service to check the load on the cacti-server machine.
